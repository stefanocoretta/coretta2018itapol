...
kinematics <- list.files(
  ...
  read_aaa(., column_names = columns, knots = 42, format = "wide") %>%
  select(-(`X1 NA`:frame_id)) %>%
...
tongue_contours <- list.files(
  ...
  read_aaa(., column_names = columns, knots = 42, format = "long")
```
...
```{r dlc-data}
dlc_data <- read_aaa("data-raw/datasets/ultrasound/dlc-ultrasound.tsv")

dlc_colnames <- colnames(dlc_data)
dlc_colnames[1:6] <- c("participant", "date", "time_rec", "time_annot", "prompt", "annotation")
colnames(dlc_data) <- dlc_colnames

knot_labels <- tibble(
  knot = c(0:10, 0:1, 0:1),
  spline = c(rep("DLC_Tongue", 11), rep("DLC_HyoidMandible", 2), rep("DLC_ShorttendonMandible", 2)),
  knot_label = c("Vallecula", "Root_1", "Root_2", "Body_1", "Body_2", "Dorsum_1", "Dorsum_2", "Blade_1", "Blade_2", "Tip_1", "Tip_2", "Hyoid", "Mandible", "Short_tendon", "Mandible")
)

dlc_data <- dlc_data |> 
  left_join(y = knot_labels) |> 
  mutate(
    word = word(prompt, 2),
    vowel = str_sub(word, 2, 2),
    consonant = str_sub(word, 3, 3),
    consonant = ifelse(consonant == "c", "k", consonant),
    place = case_when(
      consonant %in% c("t", "d") ~ "coronal",
      consonant %in% c("k", "g") ~ "velar"
    ),
    voicing = case_when(
      consonant %in% c("t", "k") ~ "voiceless",
      consonant %in% c("d", "g") ~ "voiced"
    ),
    language = case_when(
      str_detect(participant, "pl") ~ "Polish",
      str_detect(participant, "it") ~ "Italian"
    )
  )
```
...
dlc_voff <- read_aaa(
  "data-raw/datasets/ultrasound/dlc-voffset.tsv",
  knots = 11
)

dlc_colnames <- colnames(dlc_voff)
dlc_colnames[1:6] <- c("speaker", "date", "time_rec", "time_annot", "prompt", "annotation")
colnames(dlc_voff) <- dlc_colnames
...
usethis::use_data(dlc_data, compress = "xz", overwrite = TRUE)
usethis::use_data(dlc_voff, overwrite = TRUE)
```

Save aggregated `csv` files for use in other software.

```{r aggregate}
write_csv(token_measures, file = "./data-raw/aggregated/token-measures.csv")
write_csv(kinematics, file = "./data-raw/aggregated/kinematics.csv")
write_csv(formants_series, file = "./data-raw/aggregated/formants-series.csv")
write_csv(tracegram, file = "./data-raw/aggregated/tracegram.csv")
write_csv(tongue_contours, file = "./data-raw/aggregated/tongue-contours.csv")
write_csv(kinematics_series, file = "./data-raw/aggregated/kinematics-series.csv")
write_csv(wavegram, file = "./data-raw/aggregated/wavegram.csv")
```
